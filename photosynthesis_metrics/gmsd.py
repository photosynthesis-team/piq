...
class GMSDLoss(_Loss):
    ...
    def forward(self, prediction: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        """
        _validate_input(input_tensors=(prediction, target), supports_5d=False, scale_weights=None)
...
class MultiScaleGMSDLoss(_Loss):
    ...
    def forward(self, prediction: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            Value of GMSD loss to be minimized. 0 <= GMSD loss <= 1.
        """
        _validate_input(input_tensors=(prediction, target), supports_5d=False, scale_weights=self.scale_weights)
        prediction, target = _adjust_dimensions(input_tensors=(prediction, target))
